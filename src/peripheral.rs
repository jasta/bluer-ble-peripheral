...
use ble_peripheral::gatt_server_cb::{AdvStartFailedReason, AdvStopReason, GattServerCallback, GattServerEvent};
use tokio::sync::mpsc;
use std::sync::Arc;
...
impl Peripheral for BluerPeripheral {
    ...
    fn configure_gatt_server(
        ...
        let advertiser = BluerAdvertiser {
            tx: tx.clone(),
            service_uuids: Arc::new(services.iter().map(|s| uuid_util::convert_uuid(s.uuid)).collect()),
