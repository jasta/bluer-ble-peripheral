...
use bluer::gatt::local::{Application, Characteristic, CharacteristicNotify, CharacteristicNotifyFun, CharacteristicNotifyMethod, CharacteristicRead, CharacteristicReadRequest, CharacteristicWrite, CharacteristicWriteMethod, CharacteristicWriteRequest, Descriptor, DescriptorRead, DescriptorReadRequest, DescriptorWrite, DescriptorWriteRequest, ReqError, ReqResult, Service};
use ble_peripheral::descriptors::{AttributeHandle, GattCharacteristic, GattCharacteristicPermission, GattCharacteristicProperty, GattDescriptor, GattDescriptorPermission, GattService, GattServiceType, UUID};
    ...
    fn new_descriptor(&mut self, spec: &GattDescriptor) -> Result<Descriptor, bluer::Error> {
        ...
        for perm in spec.permissions {
            match perm {
                ...
                GattDescriptorPermission::WriteSigned => Err(err_util::generic_err("write signed not supported"))?,
                GattDescriptorPermission::WriteSignedMitm => {
                    Err(err_util::generic_err("write signed mitm not supported"))?
                }
            }
